...
class ScreenCapture:
    ...
    def capture_question_area(self) -> Optional[np.ndarray]:
        ...
        # Extract question area (typically top 30% of screen, skip browser UI)
        height, width = full_capture.shape[:2]
        # Skip top 10% to avoid browser UI, then take 30% for question
        start_y = int(height * 0.1)
        end_y = int(height * 0.4)
        question_area = full_capture[start_y:end_y, 0:width]
    ...
    def capture_answers_area(self) -> Optional[np.ndarray]:
        ...
        # Extract answers area (bottom portion, skip browser UI at top)
        height, width = full_capture.shape[:2]
        # Start from 40% down, go to 90% to avoid bottom browser UI
        start_y = int(height * 0.4)
        end_y = int(height * 0.9)
        answers_area = full_capture[start_y:end_y, 0:width]
        return answers_area
